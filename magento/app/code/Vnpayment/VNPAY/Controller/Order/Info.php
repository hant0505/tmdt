...
class Info extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if ($order->getId()) {
            ...
            $amountUsd = $order->getTotalDue();
            $baseCurrency = $this->storeManager->getStore()->getBaseCurrencyCode();
            if ($baseCurrency !== 'VND') {
                try {
                    $currency = $this->storeManager->getStore()->getBaseCurrency();
                    $amountVnd = $currency->convert($amountUsd, 'VND');
                } catch (\Exception $e) {
                    $amountVnd = $amountUsd * 25000; // fallback nếu chưa set tỷ giá
                }
            } else {
                $amountVnd = $amountUsd;
            }
            ...
            $inputData = array(
                ...
                "vnp_Amount" => round($amountVnd * 100, 0),
                "vnp_Command" => "pay",
